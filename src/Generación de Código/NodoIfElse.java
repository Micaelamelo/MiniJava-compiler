...
public class NodoIfElse extends NodoSentencia{
	...
	public void generar(TablaDeSimbolos t) {
		expresion.generar(t);		
		String l1 = t.getEtiqueta();
		t.generarInstruccion("BF "+ l1);
	
		sentencia.generar(t);
		
		//System.out.println("sentencia else es: "+sentenciaElse);
		
		if (sentenciaElse == null) {
			t.generarInstruccion(l1+": NOP");
		}
		else { //no anda :( 
			String l2 = t.getEtiqueta();
			t.generarInstruccion("JUMP "+l2);
			t.generarInstruccion(l1+": NOP");
			sentenciaElse.generar(t);
			
			t.generarInstruccion(l2+": NOP");
		}	
	}
}
