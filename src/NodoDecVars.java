...
public class NodoDecVars extends NodoSentencia{
	...
	public void chequear() throws Exception{
		...
		for(Atributo e : variables) {		
			System.out.println("voy a agregar la variable "+e.getNombre());
			Tipo t = e.getTipo();		
			if (t instanceof TIdClase){
				if (tabla.existeClase(t.getTipo())) {
					...
					throw new Exception("Error semántico, el tipo de la variable '"+e.getNombre()+"' es de una clase que no existe. Linea "+e.getNroLinea());
			}
			else 
				bloque.insertarVariableLocal(e);				
		}
